...
//Step 4: 執行 SQL 指令	
		...
		rs5.next();
		   ...
		   String refuse_reason = request.getParameter("refuse_reason");
		   out. print(refuse_reason);  
			   sql="UPDATE `application` SET `certification_status`='認證失敗',`Rejection`='"+refuse_reason+"' WHERE `ID` =(";
			   sql+="SELECT `ID` FROM (";
			   ...
			   sql+="FROM project.application WHERE `certification_ID`= "+ rs5.getString(1) +" AND `audit_status`='審核成功' AND `certification_status` is null";
			   ...
			   sql+=")AS nothing WHERE `apply_num`="+getvalue+")";
			   sql+="AND";
			   sql+="`audit_status`='審核成功'";
			   sql+="AND";
			   sql+="`certification_status` is null";    
			   con.createStatement().execute(sql);
...
//Step 5: 顯示結果 
          //直接顯示最新的資料
          response.sendRedirect("T_time-apply.jsp"); 
