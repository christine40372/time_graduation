...
%>   
  ...
  <script> 
	...
	function getV(n){
		...
		document.form1.action="T_time-apply_add.jsp";
	...
	function refuse(n){
		...
		getvalue.value=counttable;
		console.log(getvalue.value); //可以在f12中的主控台看到取的按鈕號碼有沒有對
		document.form1.action="T_Refuse.jsp";
		document.form1.submit();
	}	  
  </script> 
...
<%
			...
			if (session.getAttribute("acc")!=null && session.getAttribute("status")=="時數認證者"){
				...
				String sql="SELECT ROW_NUMBER()  OVER (ORDER BY `service_name`,`service_date`) AS apply_num,`chinese_name`,`ID`,`unit_name`,`service_name`,`service_class`,`service_date`,TIME_FORMAT(`service_time1`,'%H:%i')AS 'service_time1',TIME_FORMAT(`service_time2`,'%H:%i') AS 'service_time2',`hours`,DATE_FORMAT(`apply_date`,'%Y-%m-%d') AS 'apply_date',`Rejection` FROM project.application WHERE `certification_ID`= "+ rs5.getString(1) +" && `audit_status`='審核成功' && `certification_status` is null";
...
%>    
	...
	<form name="form1" action="" method="post">
        ...
        <table class="table table-hover" id="myTable" style="text-align: center;">
          <thead>
            <tr>
              <th style="width: 7.5ch;">志工姓名</th>
              <th style="width: 9ch;">身分證字號</th>
              <th style="width: 10ch;">服務單位</th>
              <th style="width: 8.5ch;">服務名稱</th>
              <th style="width: 12ch;">服務類別</th>
              <th style="width: 9ch;cursor: pointer;" onclick="sortTable(5)">服務日期 &#8645</th>
              <th style="width: 8ch;">服務時間<br>(開始)</th>
              <th style="width: 0.5ch;"></th>
              <th style="width: 8ch;">服務時間<br>(結束)</th>
              <th style="width: 7.5ch;">服務時數</th>
              <th style="width: 9ch;">申請日期</th>
              <th style="width: 10ch;">說明</th>
              <th style="width: 10ch;">認證與否</th>
            </tr>
          </thead>
          <tbody>
<%
			while(tmp.next()){
				rejection=tmp.getString(12);						
				if(rejection==null){
					rejection="";
				}
%>
