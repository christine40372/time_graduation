...
<div style="margin-top: 70px;">
		...
		try {
				...
				//Step 2: 建立連線 	
					...
					if(con.isClosed())
						...
						String name_CH=new String(request.getParameter("name_CH").getBytes("ISO-8859-1"),"UTF-8");
						String name_EN=new String(request.getParameter("name_EN").getBytes("ISO-8859-1"),"UTF-8");
	...
	%>
							<center>
								<h3>請完整填寫欄位</h3>
								<a href="changeself_h_e.jsp" style="font-size: 20px">重新填寫</a>
							</center>
	<%
						}
						else{
							sql = "UPDATE `member` SET `chinese_name`='"+ name_CH +"' WHERE `ID`='"+ id +"'";
							con.createStatement().execute(sql);
							sql = "UPDATE `member` SET `english_name`='"+ name_EN +"' WHERE `ID`='"+ id +"'";
							con.createStatement().execute(sql);
							sql = "UPDATE `member` SET `phone`='"+ tel +"' WHERE `ID`='"+ id +"'";
							con.createStatement().execute(sql);
							sql = "UPDATE `member` SET `email`='"+ email +"' WHERE `ID`='"+ id +"'";
							con.createStatement().execute(sql);
							con.close();
							response.sendRedirect("changeself_v_end.jsp") ;
						}
						con.close();
					}
			}
