...
//Step 5: 顯示結果
						...
						String sql="SELECT  `chinese_name`,`ID`,`unit_name`,`service_name`,`service_class`,`service_content`,`service_date`,TIME_FORMAT(`service_time1`,'%H:%i')AS 'service_time1',TIME_FORMAT(`service_time2`,'%H:%i') AS 'service_time2', `certification_ID`, `s_member_ID` FROM `sing_up` WHERE `service_name` IN ( SELECT `service_name` FROM (SELECT  ROW_NUMBER()  OVER (ORDER BY`unit_name`) AS apply_num,`unit_name`,`service_name`,`service_class`,`service_sdate`,`service_edate`,`service_date`,`Rejection`,`sing_up_status`,`certification_ID`, `s_member_ID` FROM `sing_up`) AS nothing WHERE `apply_num`="+getvalue+")AND `service_date`IN ( SELECT `service_date` FROM (SELECT  ROW_NUMBER()  OVER (ORDER BY`unit_name`) AS apply_num,`unit_name`,`service_name`,`service_class`,`service_sdate`,`service_edate`,`service_date`,`Rejection`,`sing_up_status`,`certification_ID`, `s_member_ID` FROM `sing_up`) AS nothing WHERE `apply_num`="+getvalue+")";
...
%>
			...
			<input type="hidden" name="c_id" value="<%=tmp.getString(10)%>">
			<input type="hidden" name="s_m_id" value="<%=tmp.getString(11)%>">
