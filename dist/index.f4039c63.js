$(function() {
    /*----產生data-th-----*/ let $table = $("#myTable");
    let $thRows = $table.find("thead th");
    $thRows.each(function(key, thRow) {
        $table.find("tbody tr td:nth-child(" + (key + 1) + ")").attr("data-th", $(thRow).text());
    });
...
function goPage(currentPage, pageSize) {
    ...
    for(var i = 1; i < num + 1; i++){
        var trRow = tr[i - 1];
        if (i >= startRow && i <= endRow) trRow.style.display = "";
        else trRow.style.display = "none";
    }
    var tempStr = "";
    if (currentPage > 1) {
...
//# sourceMappingURL=index.f4039c63.js.map
